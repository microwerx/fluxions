...
bool    ImFontAtlas::Build()
{
    ...
    // After packing is done, width shouldn't matter much, but some API/GPU have texture_ size limitations and increasing width can decrease height.
    ...
    // Pack our extra data rectangles first, so it will be on the upper-left corner of our texture_ (UV will have small values).
    ...
    // First font pass: pack all glyphs (no rendering at this point, we are working with rectangles in an infinitely tall texture_ at this point)
    for (int input_i = 0; input_i < ConfigData.Size; input_i++)
    {
        ImFontConfig& cfg = ConfigData[input_i];
        ImFontTempBuildData& tmp = tmp_array[input_i];
