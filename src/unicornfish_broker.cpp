...
namespace Uf
{
	...
	bool Broker::ProcessWorkerMessage(Frame & sender, Message & msg)
	{
		...
		if (command == Majordomo::Command::Ready)
		{
			if (workerIsReady)
			{
				if (verbose) printf("broker: received READY from worker, but worker was ready, disconnecting worker\n");
				DisconnectWorker(worker);
			}
			else if (sender.strncmp("mmi.", 4))
			{
				// workers aren't allowed to have services that start with "mmm."
				if (verbose) printf("broker: received MMI from worker, but worker can't have MMIs, disconnecting worker\n");
				DisconnectWorker(worker);
			}
			else
			{
				if (verbose) printf("broker: received READY from worker, adding worker to waiting list\n");
		...
		else if (command == Majordomo::Command::Reply)
		{
			if (workerIsReady)
			{
				...
				if (verbose) printf("broker: received reply from worker, now sending reply to client %s\n", clientHexAddress.c_str());
				msg.Send(brokerSocket);
				WaitWorker(worker);
			}
			else
			{
				if (verbose) printf("broker: received reply from worker, but worker was not ready, disconnecting worker\n");
				DisconnectWorker(worker);
			}
		}
		else if (command == Majordomo::Command::Heartbeat)
		{
			if (workerIsReady)
			{
				if (verbose) printf("broker: received heartbeat from worker, renewing expiry\n");
				worker->RenewExpiry(zclock_time() + Majordomo::DefaultHeartbeatIntervalMS);
			}
			else
			{
				if (verbose) printf("broker: received heartbeat from worker, but worker was not ready, disconnecting worker\n");
				DisconnectWorker(worker);
			}
		}
		else if (command == Majordomo::Command::Disconnect)
		{
			if (verbose) printf("broker: received disconnect from worker, deleting worker\n");
			DeleteWorker(workerName);
		}
		else
		{
			// invalid input message
			if (verbose) printf("broker: received invalid input message\n");
		}
		return true;
	}
	...
	bool Broker::ProcessClientMessage(Frame & sender, Message & msg)
	{
		...
		if (serviceFrame.strncmp("mmi.", 4))
		{
			...
			if (verbose) printf("Broker: sending %d frames to client %s\n", (int)msg.Size(), clientAddress.c_str());
			msg.Send(brokerSocket);
		}
		else
		{
	...
	bool Broker::RunLoop()
	{
		...
		if (brokerSocket.Poll(Majordomo::DefaultHeartbeatIntervalPollMS))
		{
			...
			if (msg.Size() >= 3)
			{
				if (verbose) printf("broker: received message size is %d\n", (int)msg.Size());
				...
				else
				{
					...
					if (verbose) printf("broker: error with message\n");
				}
			}
			else
			{
				// need to do error handling here...
				if (verbose) printf("broker: error with message size\n");
			}
		}
		...
		if (zclock_time() > heartbeatTime)
		{
			...
			{
				if (verbose) printf("broker: sending heartbeat to worker %s\n", worker->key.c_str());
	...
	{
		if (verbose) printf("Broker: sending disconnect\n");
	...
	{
		auto worker_iterator = workers.find(workerName);
		if (worker_iterator == workers.end()) return;
	...
	void Broker::DispatchService(ServiceInfo * service, Message & msg)
	{
		...
		if (msg) service->requests.push_back(msg);
