...
namespace KASL
{
	...
	{
		myThread = thread(PythonInterpreter::thread_func, myProgramName, this);
	}
	...
	{
		if (currentState == State::InitialState) return false;
	...
	void PythonInterpreter::thread_func(const string &programName, PythonInterpreter *pPyInterpreter)
	{
		...
		cout << "PythonInterpreter::thread_func() [" << pPyInterpreter->myThread.get_id() << "]: starting..." << endl;
		try {
	...
	{
		if (currentState == State::InitialState) return;
	...
	int test_PythonInterpreter(int argc, char **argv)
	{
		...
		python.start();
		bool done = false;
		__int64 cmdcount = python.getCommandCount();
		while (!done)
		{
			while (cmdcount == python.getCommandCount()) this_thread::yield();
