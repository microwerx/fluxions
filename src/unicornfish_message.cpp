...
namespace Uf
{
	...
	Majordomo::Command Message::PopCommand()
	{
		...
		if (!msg) return Command::None;
		char *str = zmsg_popstr(msg);
		int value = str[0];
		lastPoppedString = str;
		zstr_free(&str);
		if (value == (int)Command::None) return Command::None;
		if (value == (int)Command::Ready) return Command::Ready;
		if (value == (int)Command::Request) return Command::Request;
		if (value == (int)Command::Reply) return Command::Reply;
		if (value == (int)Command::Heartbeat) return Command::Heartbeat;
		if (value == (int)Command::Disconnect) return Command::Disconnect;
		return Command::None;
	}
	...
	const string &Message::PopString()
	{
		...
		if (!msg) return lastPoppedString;
		char *str = zmsg_popstr(msg);
		if (str != nullptr) {
			lastPoppedString = str;
			zstr_free(&str);
	...
	Frame & Message::Unwrap()
	{
		...
		if (!msg) return lastUnwrappedFrame;
