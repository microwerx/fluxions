...
namespace Fluxions
{
	...
	bool SceneGraphWriter::write(const SimpleSceneGraph* pssg) {
		...
		for (auto& [i, sphere] : ssg.spheres) {
			if (ssg.materialSystem.GetLibraryId() != sphere.mtllibId) {
		...
		// 5. Geometry Groups
		for (const auto& [i, geo] : ssg.geometryGroups) {
			if (ssg.materialSystem.GetLibraryId() != geo.mtllibId) {
		...
		for (const auto& [i, geo] : ssg.geometryGroups) {
			ssg.staticMeshes[geo.objectId].saveOBJ(geo.objectName);
		}
