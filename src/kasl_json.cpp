...
namespace KASL
{
	...
	JSON::JSON(const JSON &json)
	{
		...
		else if (IsString()) sval_ = json.sval_;
		else if (IsArray()) copyArray(json.array_);
		else if (IsObject()) copyMap(json.map_);
	}
	...
	bool JSON::Equals(const JSONPtr &rhs) const
	{
		...
		if (type_ == Type::Null) return true;
		if (type_ == Type::True) return true;
		if (type_ == Type::False) return true;
		if (type_ == Type::Number) return dval_ == rhs->dval_;
		if (type_ == Type::String) return sval_ == rhs->sval_;
	...
	long JSON::DeserializeParseTokens(const TokenVector &tokens, long i)
	{
		for (size_t len = tokens.size(); i < len; i++)
		{
