	...
	while (1) {
		...
		if (str == "transform") {
			...
			if (filenameToTry.empty()) {
				Hf::Log.error("%s(): MTLLIB %s was not found.", __FUNCTION__,
							  mtllibFilename.c_str());
			}
			else {
				if (materials.Load(fpi.fname, filenameToTry)) {
					Hf::Log.info("%s(): MTLLIB %s loaded.", __FUNCTION__,
								 mtllibFilename.c_str());
				}
				else {
					Hf::Log.error("%s(): MTLLIB %s had an error while loading.",
								  __FUNCTION__, mtllibFilename.c_str());
				}
			}
		}
			...
			fpi.FindFileIfExists(pathsToTry, filenameToTry);
			if (filenameToTry.empty()) {
				Hf::Log.errorfn(__FUNCTION__, "OBJ file %s was not found.",
								geoFilename.c_str());
			}
			else {
		...
		for (auto mtlIt = mtllib.mtls.begin(); mtlIt != mtllib.mtls.end();
			...
			for (auto tmapIt = textures.begin(); tmapIt != textures.end(); tmapIt++) {
				...
				if (pMap) {
					...
					glTexParameterf(pMap->textureObject.GetTarget(),
...
void SimpleSceneGraph::Render(SimpleProgram& program, bool useMaterials,
	...
	program.ApplyUniform("UScreenWidth",
						 SimpleUniform((GLfloat)camera.imageWidth));
	program.ApplyUniform("UScreenHeight",
						 SimpleUniform((GLfloat)camera.imageHeight));
	...
	program.ApplyUniform("InverseCameraMatrix",
		(SimpleUniform)cameraMatrix.AsInverse());
	program.ApplyUniform(
		"SunE0",
		SimpleUniform(
			environment
			.curSunDiskRadiance)); // .ssg.SimpleUniform(sunDiskRadiance.toVector4f()));
	program.ApplyUniform(
		"GroundE0",
		SimpleUniform(
			environment
			.curGroundRadiance)); // SimpleUniform(groundRadiance.toVector4f()));
	program.ApplyUniform("SunShadowBiasMatrix",
						 SimpleUniform(environment.sunShadowBiasMatrix));
	program.ApplyUniform("SunShadowProjectionMatrix",
						 SimpleUniform(environment.sunShadowProjectionMatrix));
	program.ApplyUniform("SunShadowViewMatrix",
						 SimpleUniform(environment.sunShadowViewMatrix));
	program.ApplyUniform("SunShadowInverseViewMatrix",
						 SimpleUniform(environment.sunShadowInverseViewMatrix));
...
void SimpleSceneGraph::AdvancedRender(SimpleRenderConfiguration& rc) {
	...
	if (!rc.useZOnly) {
		...
		// (SimpleUniform)(cameraMatrix.AsInverse()).col4());
		...
		program->ApplyUniform("ProjectionMatrix",
			(SimpleUniform)rc.projectionMatrix);
		...
		if (rc.isCubeMap) {
			...
			SimpleUniform shadowmats(GL_FLOAT_MAT4, 6, 0,
									 cubeShadowMatrices[0].ptr());

			program->Use();
			program->ApplyUniform("LightPosition",
				(SimpleUniform)rc.cameraPosition.xyz());
...
void SimpleSceneGraph::SetUniforms(SimpleProgramPtr& program) {
	...
	program->ApplyUniform("EnviroColorMap",
		(SimpleUniform)environment.enviroColorMapUnit);
	...
	if (useMaps) {
		...
		for (auto tmapIt = currentTextures.begin(); tmapIt != currentTextures.end();
			...
			if (pMap) {
				...
				if (pMap->samplerId == 0) {
					glTexParameterf(pMap->textureObject.GetTarget(), GL_TEXTURE_WRAP_S,
									GL_REPEAT);
					glTexParameterf(pMap->textureObject.GetTarget(), GL_TEXTURE_WRAP_T,
									GL_REPEAT);
					glTexParameterf(pMap->textureObject.GetTarget(), GL_TEXTURE_WRAP_R,
									GL_REPEAT);
					glTexParameterf(pMap->textureObject.GetTarget(),
				...
				if (pMap->unitId < 0 || pMap->unitId >= g_MaxCombinedTextureUnits) {
					Hf::Log.error("%s(): pMap->unitId (%d) is out of range.",
								  __FUNCTION__, pMap->unitId);
				}
				else {
					if (pMap->map_loc >= 0)
...
void __ShaderProgramLocations::GetMaterialProgramLocations(
	...
	// Do the new way by finding out what uniforms the program actually has, or is
	// looking for...
