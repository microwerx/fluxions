...
namespace Fluxions
{
	...
	void SimpleSceneGraph::Render(SimpleProgram& program)
	{
		...
		for (auto sphIt = spheres.begin(); sphIt != spheres.end(); sphIt++) {
			...
			Color3f Ke = materials.SetLibraryMaterial(sphIt->second.mtllibName, sphIt->second.mtlName)->Ke;
	...
	void SimpleSceneGraph::ApplyGlobalSettingsToCurrentProgram()
	{
		...
		if (locs.toneMapExposure >= 0)
			glUniform1f(locs.toneMapExposure, 2.5f * powf(2.0f, environment.toneMapExposure));
	...
	void __ShaderProgramLocations::GetMaterialProgramLocations(SimpleProgram& program)
	{
		...
		for (int i = 0; i < 16; i++) {
			...
			locations[name] = sphl_lights_E0[i] = program.GetUniformLocation(name);
			sprintf_s(name, "SphlLights[%d].Position", i);
			locations[name] = sphl_lights_position[i] = program.GetUniformLocation(name);
			sprintf_s(name, "SphlLights[%d].LightProbeCubeMap", i);
			locations[name] = sphl_lights_lightProbeCubeMap[i] = program.GetUniformLocation(name);
			sprintf_s(name, "SphlLights[%d].EnvironmentCubeMap", i);
			locations[name] = sphl_lights_environmentCubeMap[i] = program.GetUniformLocation(name);
