...
namespace Viperfish
{
	namespace vfglut
	{
		...
		static void multibutton(int id, int x, int y, int button, int state) {
			...
			if (state == GLUT_DOWN) multitouchState[id].OnButtonDown(button);
			if (state == GLUT_UP) multitouchState[id].OnButtonUp(button);

			if (state == GLUT_DOWN) widget->OnMultiButtonDown(id, button, multitouchState[id]);
			if (state == GLUT_UP) widget->OnMultiButtonUp(id, button, multitouchState[id]);
		}
		...
		static void multientry(int id, int state) {
			...
			if (state == GLUT_ENTERED) widget->OnMultiEnter(id);
			if (state == GLUT_LEFT) widget->OnMultiLeave(id);
		}
