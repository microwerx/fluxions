...
namespace Uf
{
	...
	bool Client::WaitReply()
	{
		...
		while (retriesLeft)
		{
			if (clientSocket.Poll(Majordomo::DefaultTimeoutMS))
			{
				...
				if (verbose) printf("client: received got %d frames\n", (int)replyMessage.Size());
			...
			{
				if (verbose) printf("client: socket failed!\n");
				return false;
			}
			...
			{
				if (verbose) printf("client: failed to connect to broker\n");
				return false;
			}
		}
		return true;
	...
	bool Client::SendRequestToBroker(const char *service, const Message & msg)
	{
		...
		requestMessage.Push("");
		if (verbose) printf("client: sending %d frames to broker\n", (int)requestMessage.Size());
