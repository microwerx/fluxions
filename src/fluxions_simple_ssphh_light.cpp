...
namespace Fluxions
{
	...
	bool SimpleSSPHHLight::SaveJsonSph(const string & path)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			...
			coefs->PushBack(KASL::JSON::MakeArray(coef_f));
		}
	...
	bool SimpleSSPHHLight::ReadJsonSph(const string & path)
	{
		...
		if (json->IsObject() &&
			...
			json->getMember("coefs")->Size() == 4 &&
			json->getMember("coefs")->getElement(0)->IsArray() &&
			json->getMember("coefs")->getElement(1)->IsArray() &&
	...
	void SimpleSSPHH::VIZ()
	{
		...
		for (int i = 0; i < size_; i++)
		{
			...
			for (int j = 0; j < size_; j++)
			{
				...
				if (saveJSONs) H[i][j].SaveJSON(basename + ".json");
	...
	void SimpleSSPHH::HIER(bool includeSelf, bool includeNeighbor, int MaxDegrees)
	{
		...
		for (int i = 0; i < size_; i++)
		{
			...
			if (saveJSONs) Sprime[i].SaveJSON(base + "_01_Sprime.json");
			...
			if (saveJSONs) self[i].SaveJSON(base + "_02_self.json");
			...
			if (saveJSONs) neighbor[i].SaveJSON(base + "_03_neighbor.json");
