...
{
	int min_intensity = 65535;
	int max_intensity = 0;
	Color4i min_value(65535, 65535, 65535, 65535);
	Color4i max_value(0, 0, 0, 0);
	for (int y = 0; y < imageHeight; y++)
	{
		for (int x = 0; x < imageWidth; x++)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				min_value.v[i] = min(color.v[i], min_value.v[0]);
				max_value.v[i] = max(color.v[i], min_value.v[0]);
				min_intensity = min(color.v[i], min_intensity);
				max_intensity = max(color.v[i], max_intensity);
			}
		}
	}
...
void TImage<ColorType>::_setImageData(unsigned int fromFormat, unsigned int fromType, unsigned int toFormat, unsigned int toType, int width, int height, int depth, void *_pixels)
{
	...
	if (fromType == GL_UNSIGNED_BYTE && toType == GL_FLOAT)
	{
		...
		for (int i = 0; i < count; i++)
		{
			for (int j = 0; j < stride; j++)
				pixels[i].v[j] = (typename ColorType::type)clamp(scaleFactor_itof * data[j], 0, 255);
			data += stride;
		}
	}
	else if (fromType == GL_FLOAT && toType == GL_UNSIGNED_BYTE)
	{
		...
		{
			for (int j = 0; j < stride; j++)
				pixels[i].v[j] = (typename ColorType::type)clamp(scaleFactor_ftoi * data[j], 0, 255);
			data += stride;
		}
	}
	else if (fromType == GL_UNSIGNED_BYTE && toType == GL_UNSIGNED_BYTE)
	{
		...
		{
			for (int j = 0; j < stride; j++)
				pixels[i].v[j] = (typename ColorType::type)data[j];
			data += stride;
		}
	}
