...
namespace Fluxions
{
	SimpleRenderConfig::SimpleRenderConfig(const std::string& name) :
		name_{ name } {
	...
	GLbitfield SimpleRenderConfig::GetClearBits() const {
	...
	bool SimpleRenderConfig::check() const {
	...
	void SimpleRenderConfig::reset() {
		HFLOGINFO("resetting %s...", name_.c_str());
		shaderProgram.reset();
