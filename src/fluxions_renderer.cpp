...
namespace Fluxions
{
	...
	bool Renderer::LoadConfig(const string &filename)
	{
		...
		basepath = fpi.dir;
		if (fpi.DoesNotExist()) return false;
		...
		while (fin)
		{
			...
			if (count >= 1 && tokens[0].IsString())
			{
				...
				switch (number)
				{
				case 0: processed = false; break;
				case 1: result = k_renderconfig(tokens); break;
				case 2: result = k_path(tokens); break;
				case 3: result = k_program(tokens); break;
				case 4: result = k_vertshader(tokens); break;
				case 5: result = k_fragshader(tokens); break;
				case 6: result = k_vertattrib(tokens); break;
				case 7: result = k_sampler(tokens); break;
				case 8: result = k_texture(tokens); break;
				case 9: result = k_fbo(tokens); break;
				case 10: result = k_renderbuffer(tokens); break;
				case 11: result = k_geomshader(tokens); break;
				default:
					processed = false;
					break;
				}
				...
				if (!processed && count >= 2)
				{
					...
					{
						if (tokens[1].IsInteger()) VarList.set_var(tokens[0].sval, tokens[1].ival);
						if (tokens[1].IsDouble()) VarList.set_var(tokens[0].sval, tokens[1].dval);
						if (tokens[1].IsStringOrIdentifier()) VarList.set_var(tokens[0].sval, tokens[1].sval);
					}
				}
			}
	...
	void Renderer::LoadShaders()
	{
		...
		for (auto it = RenderConfigs.begin(); it != RenderConfigs.end(); it++)
		{
			...
			for (auto & p : rc.programs)
			{
				...
				if (vspath.empty()) hflog.error("%s(): vertex shader %s does not exist", __FUNCTION__, p.vertshader.c_str());
				if (fspath.empty()) hflog.error("%s(): fragment shader %s does not exist", __FUNCTION__, p.fragshader.c_str());
	...
	bool Renderer::use_renderconfig(const string &name)
	{
		...
		pcur_renderconfig = nullptr;
		if (it == RenderConfigs.end()) return false;
	...
	{
		if (pcur_renderconfig == nullptr) return false;
	...
	bool Renderer::k_renderconfig(const KASL::TokenVector &args)
	{
		...
		if (count < 2 || args[0].sval != "renderconfig") return false;
		if (args[1].IsStringOrIdentifier()) return new_renderconfig(args[1].sval);
		return false;
	}
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "path") return false;
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "program") return false;
		if (args[1].IsStringOrIdentifier()) new_program(args[1].sval);
		return true;
	}
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "vertshader") return false;
		if (args[1].IsStringOrIdentifier())
		{
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "fragshader") return false;
		if (args[1].IsStringOrIdentifier())
		{
			if (pcur_program != nullptr)
			{
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "geomshader") return false;
		if (args[1].IsStringOrIdentifier())
		{
			if (pcur_program != nullptr)
			{
	...
	{
		size_t count = args.size();
		if (count < 3 || args[0].sval != "vertattrib") return false;
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "sampler") return false;
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "texture") return false;
		if (count == 2 && args[1].IsString())
		{
		...
		else if (!cur_texture.empty())
		{
			if (count == 3 && args[1].IsString() && args[2].IsString())
			{
					...
					(param >= GL_TEXTURE_CUBE_MAP_POSITIVE_X && param <= GL_TEXTURE_CUBE_MAP_NEGATIVE_Z))
				{
					...
					if (!path.empty()) Textures[cur_texture].files.push_back(make_pair(param, path));
				}
			}
	...
	{
		size_t count = args.size();
		if (count < 2 || args[0].sval != "fbo") return false;
		...
		else if (pcur_fbo != nullptr && count >= 5 && args[1].IsStringOrIdentifier() && args[1].sval == "attach")
		{
			...
			if (gotTarget && gotAttachment)
			{
				...
				{
					int level = 0;
					if (count == 6 && args[5].IsInteger()) level = args[5].ival;
	...
	{
		size_t count = args.size();
		if (count < 5 || args[0].sval != "renderbuffer") return false;
		else
		{
			Renderbuffer renderbuffer;

			if (args[1].IsStringOrIdentifier()) renderbuffer.name = args[1].sval;
			if (args[2].IsStringOrIdentifier()) renderbuffer.internalformat = glNameTranslator.GetEnum(args[2].sval);
			if (args[3].IsStringOrIdentifier())
			{
				renderbuffer.width = VarList.get_var_integer(args[3].sval);
			}
			else if (args[3].IsInteger()) renderbuffer.width = args[3].ival;
			else renderbuffer.width = 0;

			if (args[4].IsStringOrIdentifier())
			{
				renderbuffer.height = VarList.get_var_integer(args[4].sval);
			}
			else if (args[4].IsInteger()) renderbuffer.height = args[4].ival;
			else renderbuffer.height = 0;

			if (count == 6 && args[5].IsInteger()) renderbuffer.samples = args[5].ival;
			else renderbuffer.samples = 1;
