...
namespace Fluxions
{
	...
	template <typename T> T flterrzero(T x)
	{
	...
	{
		for (int i = 0; i < 3; i++) rgbRadianceState[i] = nullptr;
	...
	void HosekWilkiePBSky::Delete()
	{
		try {
			...
			if (sunRadianceState) arhosekskymodelstate_free(sunRadianceState);
			sunRadianceState = nullptr;
			//for (int i = 0; i < NUM_WAVELENGTHS; i++)
			//{
	...
	Real HosekWilkiePBSky::Compute(Real theta, Real gamma, int index)
	{
		...
		if (theta < 0.0 || theta >= FX_PIOVERTWO || gamma < 0.0) return 0.0;
	...
	Real HosekWilkiePBSky::ComputeSunRadiance2(Real theta, Real gamma, int index)
	{
		...
		for (int i = 0; i < NUM_WAVELENGTHS; i++)
		{
			...
			if (isfinite(amount)) addStatisticSample(amount);
			//else cout << "BLAH!\n";
		...
		if (index == 0) return xyz.r;
		if (index == 1) return xyz.g;
		if (index == 2) return xyz.b;
		...
		if (index == 0) return sRGB.r;
		if (index == 1) return sRGB.g;
		if (index == 2) return sRGB.b;
		return 0.0f;
	}
	...
	void HosekWilkiePBSky::ComputeSunRadiance3(Real theta, Real gamma, Color4f &output)
	{
		...
		if (theta < 0.0 || theta >= FX_PIOVERTWO || gamma < 0.0) return;
		Real amount;
	...
	void HosekWilkiePBSky::ComputeSunRadiance4(Real theta, Real gamma, Color4f &output)
	{
		...
		if (isfinite(output.r)) addStatisticSample(output.r);
		if (isfinite(output.g)) addStatisticSample(output.g);
		if (isfinite(output.b)) addStatisticSample(output.b);
	}
	...
	Color4f HosekWilkiePBSky::GetSunDiskRadiance() const
	{
		...
		if (!isnormal(output.r) || output.r < 0.0f) output.r = 0.0f;
		if (!isnormal(output.g) || output.g < 0.0f) output.g = 0.0f;
		if (!isnormal(output.b) || output.b < 0.0f) output.b = 0.0f;
		if (!isnormal(output.a) || output.a < 0.0f) output.a = 0.0f;
	...
	int classifyCubeFaceFromVector(float x, float y, float z)
	{
		...
		if (sx && ax >= ay && ax >= az) return 0;
		// GL_TEXTURE_CUBE_MAP_NEGATIVE_X
		if (!sx && ax >= ay && ax >= az) return 1;
		// GL_TEXTURE_CUBE_MAP_POSITIVE_Y
		if (sy && ay >= ax && ay >= az) return 2;
		// GL_TEXTURE_CUBE_MAP_NEGATIVE_Y
		if (!sy && ay >= ax && ay >= az) return 3;
		// GL_TEXTURE_CUBE_MAP_POSITIVE_Z
		if (sz && az >= ax && az >= ay) return 4;
		// GL_TEXTURE_CUBE_MAP_NEGATIVE_Z
		if (!sz && az >= ax && az >= ay) return 5;
			...
			for (int is = 0; is < resolution; is++)
			{
				for (int it = 0; it < resolution; it++)
				{
					...
					for (int curSample = 0; curSample < nSamples; curSample++)
					{
						...
						//sampleColor.r = 0.5f*v.X + 0.5f;
						//sampleColor.g = 0.5f*v.y + 0.5f;
						//sampleColor.b = 0.5f*v.z + 0.5f;
						//sampleColor.r = sp;
						//sampleColor.g = tp;
						//sampleColor.b = 0.0f;
						color += sampleColor;
	...
	void PhysicallyBasedSky::ComputeCylinderMap(int width, int height, bool normalize, float sampleScale)
	{
		...
		// (u, v) is the texture_ coordinate in the range [0.0, 1.0]
		...
		for (i = 0; i < width; i++) {
			...
			for (j = 0; j < height; j++) {
				...
				if (nSamples > 1) color = color / (float)nSamples;
	...
	void PhysicallyBasedSky::prepareForCompute(bool resetStats)
	{
		...
		if (resetStats) pbsky.resetStatisticSamples();
	}
