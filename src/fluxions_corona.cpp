...
#define MAKE_FINITE(x)  \
	if (!isfinite((x))) \
		...
		hflog.info("%s(): %s ... max uniform scale is %f", __FUNCTION__, fpi_orig.fname, BoundingBox.MaxSize());
	...
	if (cacheDoesNotExist)
	{
		...
		fout.close();
		hflog.info("%s(): %s is finished loading.", __FUNCTION__, fpi_orig.fname);
	}
