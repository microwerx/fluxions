...
namespace Fluxions
{
	...
	SimpleGLES30Renderer::~SimpleGLES30Renderer()
	{
		 ...
		 if (eabo) glDeleteBuffers(1, &eabo);
	}
	...
	{
		if (locs.enviroCubeMap >= 0) glUniform1i(locs.enviroCubeMap, ssg.environment.enviroColorMapUnit);
		if (locs.enviroCubeMapAmount >= 0) glUniform1f(locs.enviroCubeMapAmount, 0.0);
		if (locs.pbskyCubeMap >= 0) glUniform1i(locs.pbskyCubeMap, ssg.environment.pbskyColorMapUnit);
		if (locs.sunDirTo >= 0) glUniform3fv(locs.sunDirTo, 1, ssg.environment.curSunDirTo.v);
		if (locs.sunColor >= 0) glUniform3fv(locs.sunColor, 1, ssg.environment.sunColor.v);
		if (locs.sunSize >= 0) glUniform1f(locs.sunSize, ssg.environment.sunSize);
		if (locs.sunShadowMap >= 0) glUniform1i(locs.sunShadowMap, ssg.environment.sunDepthMapUnit);
		if (locs.sunColorMap >= 0) glUniform1i(locs.sunColorMap, ssg.environment.sunColorMapUnit);
	}
	...
	void SimpleGLES30Renderer::ApplyMaterialToCurrentProgram(SimpleMaterial &mtl, bool useMaps)
	{
		...
		if (useMaps)
		{
			if (!mtl.map_Ka.empty()) currentTextures["map_Ka"] = ssg.materials.GetTextureMap(mtl.map_Ka);
			if (!mtl.map_Kd.empty()) currentTextures["map_Kd"] = ssg.materials.GetTextureMap(mtl.map_Kd);
			if (!mtl.map_Ks.empty()) currentTextures["map_Ks"] = ssg.materials.GetTextureMap(mtl.map_Ks);
			if (!mtl.map_Ke.empty()) currentTextures["map_Ke"] = ssg.materials.GetTextureMap(mtl.map_Ke);
			if (!mtl.map_Ns.empty()) currentTextures["map_Ns"] = ssg.materials.GetTextureMap(mtl.map_Ns);
			if (!mtl.map_Ns.empty()) currentTextures["map_Ni"] = ssg.materials.GetTextureMap(mtl.map_Ni);
			if (!mtl.map_Tf.empty()) currentTextures["map_Tf"] = ssg.materials.GetTextureMap(mtl.map_Tf);
			if (!mtl.map_Tr.empty()) currentTextures["map_Tr"] = ssg.materials.GetTextureMap(mtl.map_Tr);
			if (!mtl.map_bump.empty()) currentTextures["map_bump"] = ssg.materials.GetTextureMap(mtl.map_bump);
			if (!mtl.map_normal.empty()) currentTextures["map_normal"] = ssg.materials.GetTextureMap(mtl.map_normal);
			if (!mtl.PBmap.empty()) currentTextures["PBmap"] = ssg.materials.GetTextureMap(mtl.PBmap);
	...
	void SimpleGLES30Renderer::RenderSkyBox()
	{
		...
		if (uCameraMatrix >= 0) {
			...
			glUniformMatrix4fv(uCameraMatrix, 1, GL_FALSE, viewMatrix.m);
		}
		if (uWorldMatrix >= 0) {
			Matrix4f worldMatrix;
			glUniformMatrix4fv(uWorldMatrix, 1, GL_FALSE, worldMatrix.m);
		}
		if (uSunDirTo >= 0 && uSunE0 >= 0 && uGroundE0 >= 0) {
			glUniform3fv(uSunDirTo, 1, ssg.environment.curSunDirTo.v);
			glUniform4fv(uSunE0, 1, ssg.environment.curSunDiskRadiance.v);
			glUniform4fv(uGroundE0, 1, ssg.environment.curGroundRadiance.v);
		}
