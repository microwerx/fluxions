...
namespace Fluxions
{
	...
	void SimpleRenderer<IndexType, GLIndexType>::Index(IndexType index)
	{
		...
		switch (GLIndexType)
		{
		case GL_BYTE: maxIndex = 127; break;
		case GL_UNSIGNED_BYTE: maxIndex = 255; break;
		case GL_SHORT: maxIndex = 32767; break;
		case GL_UNSIGNED_SHORT: maxIndex = 65535; break;
		case GL_INT: maxIndex = 2147483647; break;
		case GL_UNSIGNED_INT: maxIndex = 4294967295; break;
		}
	...
	void SimpleRenderer<IndexType, GLIndexType>::BuildMemoryBuffers()
	{
		...
		if (!zVertices.empty()) memcpy(&vertexMemoryBuffer[bufferInfo.zVertexOffset], &zVertices[0], bufferInfo.zVertexSize);
		if (!fastVertices.empty()) memcpy(&vertexMemoryBuffer[bufferInfo.fastVertexOffset], &fastVertices[0], bufferInfo.fastVertexSize);
		if (!slowVertices.empty()) memcpy(&vertexMemoryBuffer[bufferInfo.slowVertexOffset], &slowVertices[0], bufferInfo.slowVertexSize);
		if (!zIndices.empty()) memcpy(&indexMemoryBuffer[bufferInfo.zIndexOffset], &zIndices[0], bufferInfo.zIndexSize);
		if (!Indices.empty()) memcpy(&indexMemoryBuffer[bufferInfo.IndexOffset], &Indices[0], bufferInfo.IndexSize);
	}
	...
	void SimpleRenderer<IndexType, GLIndexType>::BuildBuffers()
	{
		...
		if (!vertexMemoryBuffer.empty()) glBufferData(GL_ARRAY_BUFFER, vertexMemoryBuffer.size(), &vertexMemoryBuffer[0], GL_STATIC_DRAW);
		if (!indexMemoryBuffer.empty()) glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexMemoryBuffer.size(), &indexMemoryBuffer[0], GL_STATIC_DRAW);
		...
		glVertexAttribPointer(0, 4, GL_SHORT, GL_FALSE, sizeof(SimpleFastVertex), (GLvoid*)__int64(bufferInfo.fastVertexOffset));
		glVertexAttribPointer(1, 4, GL_SHORT, GL_FALSE, sizeof(SimpleFastVertex), (GLvoid*)__int64((bufferInfo.fastVertexOffset + 8)));
		glVertexAttribPointer(2, 2, GL_SHORT, GL_FALSE, sizeof(SimpleFastVertex), (GLvoid*)__int64((bufferInfo.fastVertexOffset + 16)));
		glVertexAttribPointer(3, 4, GL_UNSIGNED_BYTE, GL_FALSE, sizeof(SimpleFastVertex), (GLvoid *)__int64((bufferInfo.fastVertexOffset + 20)));
		glVertexAttribPointer(4, 4, GL_SHORT, GL_FALSE, sizeof(SimpleFastVertex), (GLvoid *)__int64((bufferInfo.fastVertexOffset + 24)));
		...
		for (int i = 0; i < 8; i++)
		{
			...
			glVertexAttribPointer(i, 4, GL_FLOAT, GL_FALSE, sizeof(SimpleSlowVertex), (GLvoid *)__int64(bufferInfo.slowVertexOffset + i * 16));
		}
	...
	void SimpleRenderer<IndexType, GLIndexType>::Render()
	{
		...
		for (auto surface = surfaces.begin(); surface != surfaces.end(); surface++)
		{
			...
			if (surface->isIndexed)
			{
				glDrawElements(surface->mode, surface->count, GLIndexType, (GLvoid*)__int64(surface->baseIndexBufferOffset));
			}
			else
			{
	...
	void SimpleRenderer<IndexType, GLIndexType>::RenderZOnly()
	{
		...
		for (auto surface = surfaces.begin(); surface != surfaces.end(); surface++)
		{
			...
			if (surface->isIndexed)
			{
				glDrawElements(surface->mode, surface->count, GLIndexType, (GLvoid*)__int64(surface->baseZIndexBufferOffset));
			}
			else
			{
				glDrawArrays(surface->mode, surface->first, surface->count);
	...
	void SimpleRenderer<IndexType, GLIndexType>::RenderIf(const string &objectName, const string &groupName, const string &mtllibName, const string &mtlName, bool onlyRenderZ)
	{
		...
		for (auto surface = surfaces.begin(); surface != surfaces.end(); surface++)
		{
			if (surface->vertexType == VertexType::UNDECIDED) continue;
			if (objectName.empty() != true && objectName != surface->objectName) continue;
			if (groupName.empty() != true && groupName != surface->groupName) continue;
			if (mtllibName.empty() != true && mtllibName != surface->mtllibName) continue;
			if (mtlName.empty() != true && mtlName != surface->mtlName) continue;
	...
	void SimpleRenderer<IndexType, GLIndexType>::RenderIf(GLuint objectId, GLuint groupId, GLuint mtllibId, GLuint mtlId, bool onlyRenderZ)
	{
		...
		{
			if (surface->vertexType == VertexType::UNDECIDED) continue;
			if (objectId != 0 && objectId != surface->objectId) continue;
			if (groupId != 0 && groupId != surface->groupId) continue;
			if (mtllibId != 0 && mtllibId != surface->mtllibId) continue;
			if (mtlId != 0 && mtlId != surface->mtlId) continue;
