...
namespace KASL
{
	...
	{
	public:
		using SharedPtr = shared_ptr<JSON>;
	...
	public:
		JSON(Type whichType);
		JSON(const int ival) { type_ = Type::Number; dval_ = ival; }
		JSON(const double dval) { type_ = Type::Number; dval_ = dval; }
		JSON(const std::string &sval) { type_ = Type::String; sval_ = sval; }
		
		explicit JSON(initializer_list<int> numberList)
		{
			type_ = Type::Array;
		...
		explicit JSON(initializer_list<float> numberList)
		{
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
			{
		...
		explicit JSON(initializer_list<double> numberList)
		{
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
			{
		...
		explicit JSON(initializer_list<string> stringList)
		{
			type_ = Type::Array;
		...
		explicit JSON(const vector<string> & stringList)
		{
			type_ = Type::Array;
			for (auto it = stringList.begin(); it != stringList.end(); it++)
			{
		...
		explicit JSON(initializer_list<pair<string, JSONPtr>> members)
		{
			type_ = Type::Object;
		...
		explicit JSON(const map<string, JSONPtr> & members)
		{
			type_ = Type::Object;
			for (auto it = members.begin(); it != members.end(); it++)
			{
		...
		inline JSONPtr operator = (int ival) { Clear();  dval_ = ival; type_ = Type::Number; return shared_from_this(); }
		inline JSONPtr operator = (double dval) { Clear(); dval_ = dval; type_ = Type::Number; return shared_from_this(); }
		inline JSONPtr operator = (const std::string &sval) { Clear(); sval_ = sval; type_ = Type::String; return shared_from_this(); }
		...
		inline JSONPtr set(initializer_list<int> numberList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
		...
		inline JSONPtr set(const vector<int> & numberList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
		...
		inline JSONPtr set(initializer_list<float> numberList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
		...
		inline JSONPtr set(initializer_list<double> numberList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
		...
		inline JSONPtr set(const vector<double> & numberList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = numberList.begin(); it != numberList.end(); it++)
		...
		inline JSONPtr set(initializer_list<string> stringList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = stringList.begin(); it != stringList.end(); it++)
		...
		inline JSONPtr set(const vector<string> & stringList)
		{
			Clear();
			type_ = Type::Array;
			for (auto it = stringList.begin(); it != stringList.end(); it++)
		...
		inline JSONPtr set (initializer_list<pair<string, JSONPtr>> members)
		{
			Clear();
			type_ = Type::Object;
			for (auto it = members.begin(); it != members.end(); it++)
		...
		inline JSONPtr set(const map<string, JSONPtr> & members)
		{
			Clear();
			type_ = Type::Object;
			for (auto it = members.begin(); it != members.end(); it++)
		...
		inline JSONPtr set(const JSONPtr & json)
		{
			...
			type_ = json->type_;
			if (IsNumber()) dval_ = json->dval_;
			if (IsString()) sval_ = json->sval_;
			if (IsArray()) array_ = json->array_;
			if (IsObject()) map_ = json->map_;
			return shared_from_this();
		}

		inline JSONPtr PushBack(JSONPtr & json) { if (!json || !IsArray()) throw std::runtime_error("JSON object is not an array"); array_.push_back(json); return shared_from_this(); }
		inline JSONPtr PopBack() { if (!IsArray()) std::runtime_error("JSON object is not an array"); if (array_.empty()) return JSONPtr(); JSONPtr json = array_.back(); array_.pop_back(); return json; }
