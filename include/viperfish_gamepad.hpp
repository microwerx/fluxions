...
namespace Viperfish
{
	...
	public:
		...
		float GetBitf(unsigned which) const { if (within(which, 0, 31)) return fbits[which]; return 0.0f; }
		bool GetBiti(unsigned which) const { if (within(which, 0, 31)) return ibits & (1 << which); return false; }
