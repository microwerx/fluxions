...
namespace Fluxions
{
	...
	public:
		...
		template <typename U> constexpr auto operator = (const TVector4<U> & V) noexcept
		{
			x = V.x;
			y = V.y;
			z = V.z;
		...
		template <typename U> constexpr auto operator = (const TVector3<U> & V) noexcept
		{
			x = V.x;
			y = V.y;
			z = V.z;
		...
		template <typename U> constexpr auto operator += (const U val) noexcept
		{
			x += val;
			y += val;
			z += val;
		...
		template <typename U> constexpr auto operator += (const TVector2<U> & V) noexcept
		{
			x += V.x;
			y += V.y;
			return *this;
		...
		constexpr auto swizzle(IndexType v0, IndexType v1) const noexcept { return TVector2<T>(v[v0], v[v1]); }
		constexpr auto swizzle(IndexType v0, IndexType v1, IndexType v2) const noexcept { return TVector3<T>(v[v0], v[v1], v[v2]); }
		constexpr auto swizzle(IndexType v0, IndexType v1, IndexType v2, IndexType v3) const noexcept { return TVector4<T>(v[v0], v[v1], v[v2], v[v3]); }
		...
		template <typename U> constexpr T dot(const TVector4<T> & v2) const noexcept
		{
