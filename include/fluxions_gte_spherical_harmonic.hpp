...
namespace Fluxions
{
	...
	template <typename T> T calc_spherical_harmonic(int max_degree, const T *a, T theta, T phi);
	...
	public:
		...
		{
			if (m < 0) return VectorType(FX_SQRT2 * sin(-m * phi));
			if (m > 0) return VectorType(FX_SQRT2 * cos(m * phi));
			return VectorType(1);
		}
	...
	struct Sph4f
	{
		...
		Sph4f() { }
		Sph4f(int degree, float value = 0.0f) { for (int i = 0; i < 4; i++) msph[i].resize(degree, value); maxDegree = degree; }
		Sph4f(const Sph4f & sph) { for (int i = 0; i < 4; i++) msph[i] = sph.msph[i]; }
		Sph4f & operator = (const Sph4f & sph) { for (int i = 0; i < 4; i++) msph[i] = sph.msph[i]; return *this; }
		...
		void resize(int degree, float value = 0.0f) { for (int i = 0; i < 4; i++) msph[i].resize(degree, value); maxDegree = degree; }
		void reset() { for (int i = 0; i < 4; i++) msph[i].resize(maxDegree, 0.0f); }
		...
		void Accumulate(const Sph4f & b, const float c, int maxDegrees = -1) { for (int i = 0; i < 4; i++) msph[i].Accumulate(b[i], c, maxDegrees); }
		void Accumulate(const Sph4f & b, const Sph4f & c, int maxDegrees = -1) { for (int i = 0; i < 4; i++) msph[i].Accumulate(b[i], c[i], maxDegrees); }
