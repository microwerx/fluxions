...
namespace Fluxions
{
	...
	public:
		...
		template <typename T2> constexpr const TVector2<T> & operator = (const TRect<T2> & rect) noexcept
		{
			x = (T)rect.x;
			y = (T)rect.y;
			w = (T)rect.w;
		...
		template <typename Other> constexpr operator TRect<Other>() const noexcept {
		...
		constexpr TRect<T> GetQuadrant(Quadrant quadrant, const TVector2<T> splitPoint) const  noexcept {
			if (quadrant == UpperLeft)  return GetUpperLeftRect(splitPoint);
			if (quadrant == UpperRight) return GetUpperRightRect(splitPoint);
			if (quadrant == LowerLeft)  return GetLowerLeftRect(splitPoint);
			if (quadrant == LowerRight) return GetLowerRightRect(splitPoint);
			return TRect<T>();
		}
