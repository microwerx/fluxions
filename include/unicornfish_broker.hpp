...
namespace Uf
{
	...
	public:
		...
		int const GetNumRequests() const { int count = 0; for (auto & service : services) count += (int)service.second.requests.size(); return count; }
	private:
		struct Worker;
		struct ServiceInfo;
		...
		struct ServiceInfo
		{
			...
			Worker * PopWorker()
			{
				if (waitingWorkers.empty()) return nullptr;
